...
function load(){
	...
	layui.config({
		...
		tableSelect.render({
			...
			searchKey: 'realname',
			table: {
				url: LOCAL_ADDRESS+'operator/getOperatorListByCompanyIdAndTypeAndRealname?companyId='+userInfo.company.id+'&type=4',
				cols: operator_col,
				limit: 5,
				limits: [5, 10, 15],
				request: {
    ...
    function submit(data, parent){
    	var url = config.elevatorBackUrl + "testing/modifyOrArrangeTestingTask"
