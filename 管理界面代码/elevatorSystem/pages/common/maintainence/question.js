...
function load(){
	...
	var col = [[
		...
		{field: 'question', title: '维护保养条目/问题', width: 400},
	...
	layui.use(['layer', 'table'], function() {
		...
		var tableIns = table.render({
			elem: '#demo',
			height: 'full-125',
			url: LOCAL_ADDRESS+'question/getQuestionListByKeyword?keyword=', 
			title: '维护问题列表',
			totalRow: true, 
			cols: col,
			page: true, 
	...
	function search(tableIns, keyword){
		var url = config.elevatorBackUrl + 'question/getQuestionListByKeyword?keyword=' + keyword; 
	...
	function edit(data){
		layer.open({
			...
			shade: 0.2,
			anim: 0,
			content: 'question-mod.html?id=' + data.id,
			area: [750 + 'px', 500 + 'px']
		});
	}
	// 【删除】
	function remove(tableIns, data){
		var url = config.elevatorBackUrl + 'question/removeQuestion?id='+data.id;
