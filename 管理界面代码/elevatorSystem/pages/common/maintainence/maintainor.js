...
function load(){
	...
	layui.use(['layer', 'table'], function() {
		...
		var tableIns = table.render({
			elem: '#demo',
			height: 'full-125',
			url: LOCAL_ADDRESS+'operator/getOperatorListByCompanyIdAndTypeAndRealname?companyId='+userInfo.company.id+'&type=3&realname=', 
			title: '管理员表',
			totalRow: true, 
			cols: col,
			page: true, 
	...
	function add(data){
		layer.open({
            ...
            content: 'maintainor-add.html',
        });
	}
	// 【搜索】
	function search(tableIns, keyword){
		var url = config.elevatorBackUrl + 'operator/getOperatorListByCompanyIdAndTypeAndRealname?companyId='+userInfo.company.id+'&type=3&realname=' + keyword; 
	...
	function edit(data){
		layer.open({
			...
			shade: 0.2,
			anim: 0,
			content: 'maintainor-mod.html?id=' + data.id,
			area: [750 + 'px', 500 + 'px']
		});
	}
	// 【删除】
